# OPERATING_MANUAL.md — Response Discipline Canon (Layer 2)

> This document is the **canonical text of Opable's Layer 2 (response discipline)**. Origin: the response method of the leaked Fable 5, observed and written down — not rules invented for a harness, but the way Fable 5 was seen to answer, codified. The full original follows below, unmodified.

## How this document integrates with the pack

Each section is deployed into the loop and the gates. On any conflict over **procedure**, this document is the canon. Output-surface rules (what appears in the visible answer) are read together with INV-7 and INV-8: every procedure here runs internally, and §7 is not an imposed template but a precise spec of the native Fable 5 norm (ADR-0003).

| OM section | Deployed into |
|---|---|
| §1 Read the request beneath the words | `phases/S0_frame.md` · `CLAUDE.md` request reading |
| §2 Independently checkable pieces | `phases/S1_decompose.md` |
| §3 Effort by cost-of-error | `EFFORT.md` |
| §4 Re-derive everything | `phases/S4_body.md` · INV-9 |
| §5 Separate registers | INV-10 · `phases/S5_verify.md` |
| §6 Attack your own conclusion | `phases/S5_verify.md` |
| §7 Answer first | `phases/S3_draft.md` · `CLAUDE.md` output style (ADR-0003: spec of the native norm) |
| §8 Mistakes that look like competence | `RUNBOOK.md` #11–19 |
| Pre-send self-test | `gates/DOD_GUIDE.md` |
